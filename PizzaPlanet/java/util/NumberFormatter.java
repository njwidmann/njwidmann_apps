package com.application.nick.pizzaplanet.util;

/**
 * Created by Nick on 6/19/2015.
 */
public class NumberFormatter {
    ...
    Examples:
    f(1000000, 0) = 1mil
    f(2500000000, 1) = 2.5bil
    f(1234567, 3) = 1.234mil
    f(123456, 0) = 123,456
     */
    public static String convertNumberToFriendlyString(long number, int precisionLevel) {
        String[] numberStrings = {"mil", "bil", "tril", "quad", "quin", "sext", "sept"};

        if(number >= 1000000) {
            String numberString = String.valueOf(number);

            int numDigits = numberString.length();
            int numSetsOfThreeDigits = numDigits / 3;
            int numDigitsLeftover = numDigits % 3;
            if(numDigitsLeftover == 0) {
                numSetsOfThreeDigits -= 1;
                numDigitsLeftover = 3;
            }

            String returnString = numberString.substring(0, numDigitsLeftover);
            if(precisionLevel > 0) {
                returnString += ".";
                returnString += numberString.substring(numDigitsLeftover, numDigitsLeftover + precisionLevel);
            }
            returnString += numberStrings[numSetsOfThreeDigits - 2];

            return returnString;

        } else {
            return commaize(number);
        }
    }

    /**
     * This method returns a number with a certain amount of digits (n)
     * Examples:
     * if n = 4
     * f(1234567) = 1.234mil
     * f(12345678) = 12.34mil
     * f(123456789) = 123.4mil
     * @param number the number to simplify
     * @return the simplified number
     */
    public static String convertNumberToFriendlyString(long number) {
        ...
        int precision = 4 - numDigitsLeftover;

        return convertNumberToFriendlyString(number, precision);
    }
    ...
    public static void main(String[] args) {
        //test addNumberStrings()

        long[] numbersToTest = {1000000l, 123456l, 1234567l, 12345678l, 1234567890l, 1234567890123456l};
        for(long number : numbersToTest) {
            System.out.println(convertNumberToFriendlyString(number));
        }
